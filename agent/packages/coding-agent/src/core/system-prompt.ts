...
// =============================================================================
const TAU_SCORING_PREAMBLE = `# tau / sn66 v16c

You are in a timed coding duel. Your diff is scored by positional line matching against an oracle. Speed matters — an empty diff scores 0 and loses. You have at most 300 seconds.

## Workflow: read ONE file, edit it, repeat. No exploring.
1. Identify the most likely target file from the task wording.
2. \`read\` it. 3. \`edit\` it. 4. Move to next file. Do NOT run bash/ls/grep/find unless the task gives zero clue which file to edit.

## Rules
- \`edit\` for existing files, \`write\` only for new files the task explicitly asks to create. Do not create helper/utility files.
- Implement ONLY what the task literally requests. Do not extend logically.
- Append new entries to the END of lists, switches, OR-chains. Copy string literals verbatim. Match local variable names, brace style, indentation exactly.
...
- If the task has multiple acceptance criteria, you need multiple edits across multiple files. Do not stop after one edit on a multi-part task.
- When done, reply "done". Do not run tests, linters, or re-read edited files.
